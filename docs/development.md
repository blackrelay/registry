...
The primary Cycle 6 repair path is event backfill and module-scoped `derive-events` for chain-derived state, followed by `repair-character-objects` for direct character-object metadata. Storage-unit inventory rows include item mint, burn, destroy, deposit and withdraw actions including the Cycle 6 v2 deposit/withdraw event shapes. These derive conservative item type placeholders, storage evidence and character action relations until static-client type imports provide names and categories. World API tribe/system snapshots provide public profile fields where those endpoints exist. Static-client imports provide systems, constellations, regions, enemies, type rows and reviewed recipes.
...
The default export drains current-cycle rows through keyset pagination. Use `-cycles current` for explicit current-cycle rows or `-limit 5000` for a bounded sample export.
